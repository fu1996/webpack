...
class AsyncQueue {
	 ...
	 */
	add(item, callback) {
		console.log("向队列%s中添加任务", this._name);
		...
		this.hooks.beforeAdd.callAsync(item, err => {
			...
			if (this._stopped) {
				...
				if (root._willEnsureProcessing === false) {
					...
					// 添加一个定时器执行函数 将回调丢入异步队列 关于事件循环 https://www.cnblogs.com/dennisj/p/12550996.html
	...
	_startProcessing(entry) {
		console.log("queue _startProcessing");
		this.hooks.beforeStart.callAsync(entry.item, err => {
			...
			try {
				// 如果是模块则 this._processor 绑定了 Compilation 中的 _factorizeModule 方法
				// 如果是路径则 this._processor 绑定了 FileSystemInfo 中的 _readFileHash 方法
				this._processor(entry.item, (e, r) => {
					inCallback = true;
					this._handleResult(entry, e, r); // 处理解析过的 入口数据
				});
			} catch (err) {
