...
class WebpackCLI {
	...
	async tryRequireThenImport(module, handleError = true) {
		let result;
		console.log(this.colors.bgBlue("tryRequireThenImport " + module));
		try {
	...
	makeOption(command, option) {
		...
		if (mainOption.type.size > 1 && mainOption.type.has(Boolean)) {
			mainOption.flags = `${mainOption.flags} [value${
				mainOption.multiple ? "..." : ""
			}]`;
	...
	getBuiltInOptions() {
		...
		const coreFlags = this.webpack.cli
			? Object.entries(this.webpack.cli.getArguments()).map(([flag, meta]) => {
					const inBuiltIn = builtInFlags.find(
						builtInFlag => builtInFlag.name === flag
					);
	...
	async run(args, parseOptions) {
		console.log(
			this.colors.bgGreen(
				`输入的命令行配置为：${JSON.stringify(
					args,
					null,
					2
				)}，读取的配置有${JSON.stringify(parseOptions, null, 2)}`
			)
		);
		...
		console.log(this.colors.bgCyan(JSON.stringify(knownCommands, null, 2)));
			...
			if (isBuildCommandUsed || isWatchCommandUsed) {
				await this.makeCommand(
					...
					async (entries, options) => {
						...
						console.log(this.colors.redBright("webpack starting..."));
				...
				if (name) {
					...
					if (operand) {
						...
						command.options.forEach(option => {
							if (
								!option.hidden &&
								levenshtein.distance(name, option.long.slice(2)) < 3
							) {
		...
		const outputVersion = async options => {
			...
			this.logger.raw(
				`webpack: ${webpack ? webpack.version : "not installed"}`
			);
			...
			try {
				devServer = await this.loadJSONFile(
	...
	async loadConfig(options) {
		...
		console.log(this.colors.bgCyan("why this use WeakMap ?"));
		if (options.config && options.config.length > 0) {
			console.log(this.colors.bgGreen("starting loading configuration"));
	...
	async buildConfig(config, options) {
		...
		console.log(this.colors.bgBlue("tryRequireThenImport CLIPlugin"));
		...
		const internalBuildConfig = item => {
					...
					if (configPath) {
						...
						if (Array.isArray(configPath)) {
							configPath.forEach(oneOfConfigPath => {
								item.cache.buildDependencies.defaultConfig.push(
									oneOfConfigPath
								);
							});
						} else {
			...
			const statsForWebpack4 =
				this.webpack.Stats && this.webpack.Stats.presetToOptions;
	...
	isValidationError(error) {
		...
		const ValidationError =
			this.webpack.ValidationError ||
			this.webpack.WebpackOptionsValidationError;

		return error instanceof ValidationError || error.name === "ValidationError";
	}

	async createCompiler(options, callback) {
		console.log(this.colors.bgGreen("Creating compiler is working..."));
		if (typeof options.nodeEnv === "string") {
			process.env.NODE_ENV = options.nodeEnv;
		}
		console.log(this.colors.bgGreen("start loadConfig"));
		let config = await this.loadConfig(options);
		console.log(this.colors.bgGreen("start buildConfig"));
		config = await this.buildConfig(config, options);

		let compiler;

		try {
			console.log(this.colors.bgGreen("start create compiler"));
	...
	async runWebpack(options, isWatchCommand) {
		...
		console.log(this.colors.redBright("Starting createCompiler"));
