...
// OptionsApply 子类 里 定义了一个 process 方法
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		if (options.externals) {
			// 配置了外部扩展 https://webpack.docschina.org/configuration/externals#root
		...
		// 为特定的 target 添加 externals 的 预设 https://webpack.docschina.org/configuration/externals/#externalsPresets
		...
		// 打出的 chunk 的格式 根据不同的配置 注入不同的插件 https://webpack.docschina.org/configuration/output/#outputchunkformat
		...
		// bundle 中是否会引入 相关的 注释 https://webpack.docschina.org/configuration/output/#outputpathinfo
		...
		// clean 是否清除之前的产物 https://webpack.docschina.org/configuration/output/#outputclean
		...
		console.log("EntryOptionPlugin 入口分析");
		new EntryOptionPlugin().apply(compiler);
		console.log("hook entryOption called");
		compiler.hooks.entryOption.call(options.context, options.entry);
		console.log("RuntimePlugin 运行时注入");
		...
		if (options.optimization.nodeEnv) {
			...
			console.log(
				"通过 DefinePlugin 注入 process.env.NODE_ENV",
		...
		console.log("hook afterPlugins called");
		compiler.hooks.afterPlugins.call(compiler);
		if (!compiler.inputFileSystem) {
			throw new Error("No input filesystem provided");
		}
		console.log("resolverFactory resolveOptions hook called");
		...
		console.log("hook afterResolvers called");
