...
class Compiler {
	...
	constructor(context, options = /** @type {WebpackOptions} */ ({})) {
		console.log("creating compiler starting 😄");
	...
	getAllHooks() {
		...
		Object.keys(this.hooks).forEach(key => {
			const realHook = this.hooks[key];
			const hookPlugin = realHook.taps.map(value => ({
				type: value.type,
				name: value.name
			}));
			if (hookPlugin.length > 0) {
				hooks[key] = hookPlugin;
			}
		});
		console.log("allCallHooks", hooks);
	}
	/**
	 * 环境初始化完毕了，开始run 起来了
	 ...
	 */
	run(callback) {
		console.log("compiler run start");
		// 是否运行中
		if (this.running) {
		...
		const onCompiled = (err, compilation) => {
			...
			console.log("hook shouldEmit called");
			this.getAllHooks();
			if (this.hooks.shouldEmit.call(compilation) === false) {
				compilation.startTime = startTime;
				compilation.endTime = Date.now();
				const stats = new Stats(compilation);
				console.log("hook done called");
				...
				this.emitAssets(compilation, err => {
					...
					if (compilation.hooks.needAdditionalPass.call()) {
						...
						this.hooks.done.callAsync(stats, err => {
							...
							if (err) return finalCallback(err);
							console.log("hook additionalPass called");
		...
		const run = () => {
			...
			this.hooks.beforeRun.callAsync(this, err => {
				...
				this.hooks.run.callAsync(this, err => {
					// 有报错的情况
					if (err) return finalCallback(err);
					// 判断 readRecords 是否使用，最后的结果都是执行 this.compile(onCompiled);
...
${other}`);
	...
	createNormalModuleFactory() {
		...
		// 调用 创建普通模块工厂 的hook normalModuleFactory
	 ...
	 */
	compile(callback) {
		// this.newCompilationParams(); 创建普通模块 normalModuleFactory 和 上下文模块 contextModuleFactory，赋值给 params
		const params = this.newCompilationParams();
		// 调用 beforeCompile 的hook，传入 params
		this.hooks.beforeCompile.callAsync(params, err => {
			if (err) return callback(err);
			// 调用 compile 入参是 params
			...
			// 传入 构建的 compilation 调用  make  hook
			this.hooks.make.callAsync(compilation, err => {
				...
				this.hooks.finishMake.callAsync(compilation, err => {
					...
					process.nextTick(() => {
						...
						// 调用 compilation 的 finish 方法
						compilation.finish(err => {
							logger.timeEnd("finish compilation");
							if (err) return callback(err);

							logger.time("seal compilation");
							// 调用 compilation 的 seal 方法
							compilation.seal(err => {
								logger.timeEnd("seal compilation");
								if (err) return callback(err);

								logger.time("afterCompile hook");
								// 调用 afterCompile 的hook
