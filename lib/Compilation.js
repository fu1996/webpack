...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	_addModule(module, callback) {
		// 唯一标识 也就是 module 的绝对路径：/webpack/examples/01-base/src/index.js
		...
		this._modulesCache.get(identifier, null, (err, cacheModule) => {
			...
			this._modules.set(identifier, module); // 将 identifier （是唯一的文件路径），和其 根据文件内容解析构成的 module 进行对应的上
	...
	_processModuleDependencies(module, callback) {
		...
		const onDependenciesSorted = err => {
			...
			// 这是嵌套的，因此我们需要允许一个额外的任务
			// increaseParallelism 方法内部是 setImmediate(root._ensureProcessing) 【异步队列】;
			this.processDependenciesQueue.increaseParallelism();
			// 对排序后依赖进行 模块构建，就像对入口处 做 模块构建一样
	...
	handleModuleCreation(
		...
		console.log("handle Module Creation");
		const moduleGraph = this.moduleGraph;
		...
		this.factorizeModule(
			...
			(err, factoryResult) => {
				...
				this.addModule(newModule, (err, module) => {
					...
					// 模块加入 模块图中了 ModuleGraph :https://webpack.docschina.org/blog/2020-10-10-webpack-5-release/#module-and-chunk-graph
					this._handleModuleBuildAndDependencies(
						originModule,
						module,
						recursive,
	...
	_addEntryItem(context, entry, target, options, callback) {
		...
		console.log("compilation addEntry hook called");
